<template>
    <aside :class="['sidebar-area', { 'collapsed': isCollapsed }]">
        ...
        <div class="sidebar-footer">
            <div class="user-profile" @click="toggleProfileMenu" :title="isCollapsed && !isMobile ? userName : ''">
            ...
            <button class="logout-btn" @click="logout" :disabled="isLoggingOut" :title="isCollapsed && !isMobile ? 'Déconnexion' : ''">
...
export default {
    ...
    setup(props, { emit }) {
        ...
        watch(() => props.isCollapsedProp, (val) => {
            if (val !== null && !isMobile.value) isCollapsed.value = val;
        });

        const checkMobile = () => {
            isMobile.value = window.innerWidth < 768;
            if (isMobile.value) {
                // Sur mobile, on désactive le collapse pour afficher tout le texte en bas
        ...
        const navigateTo = (routeName) => {
            ...
            // Le défilement est naturel maintenant sur mobile, on ramène juste en haut de page
            if (isMobile.value) {
...
.brand h4 {
    font-size: 1.25rem;
    font-weight: 700;
    color: #ffffff; /* Ajusté pour contraster avec var(--primary-color) */
    margin: 0;
    transition: opacity 0.2s ease, width 0.3s ease;
}

/* Cacher les textes en mode collapsed (Uniquement sur Desktop) */
...
.nav-item:not(.active):hover .nav-link { color: #ffffff; }
...
@media (max-width: 768px) {
    .sidebar-area {
        /* On remet la sidebar dans le flux standard, le CSS Grid du composant parent s'en occupe */
        position: relative;
        width: 100%;
        height: auto; /* Permet à la zone de prendre toute la place de son contenu */
        min-height: auto;
    }

    /* Override de sécurité: au cas où, on force l'affichage de tous les éléments textuels */
    .sidebar-area.collapsed .brand h4,
    .sidebar-area.collapsed .nav-text,
    .sidebar-area.collapsed .nav-badge,
    .sidebar-area.collapsed .nav-count,
    ...
    /* On cache le petit bouton de collapse vu que ce n'est plus pertinent sur mobile */
    .collapse-btn {
        display: none;
    }
}
