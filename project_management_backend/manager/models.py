...
class Dossier(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['avocat_responsable', 'statut']),
...
def document_upload_path(instance, filename):
    """
    Génère le chemin de stockage du document
    Format: documents/ANNEE/MOIS/TYPE_DOSSIER/REFERENCE_DOSSIER/CATEGORIE/filename
    """
    date = timezone.now()
    dossier_ref = instance.dossier.reference_dossier if instance.dossier else 'sans_dossier'
    categorie = instance.categorie.code if instance.categorie else 'autre'
    ...
    return f"documents/{date.year}/{date.month:02d}/{dossier_ref}/{categorie}/{safe_filename}"
...
class EtapeDossier(models.Model):
    """
    Modèle pour gérer les étapes d'un dossier (ajouté pour cohérence avec la référence dans Document)
    """
    dossier = models.ForeignKey(
        Dossier,
        on_delete=models.CASCADE,
